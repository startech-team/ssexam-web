        ...
        <div class="col-12">
            ...
            <div class="card shadow mb-4">
                <div class="card-header padding-x">
                    <h6 class="m-0 font-weight-bold text-primary">アカウント登録</h6>
                </div>
                <div class="card-body">
                    ...
                    <form action="{{ url('/admin/account/store') }}" method="POST">
                        ...
                        <div class="mb-0">
                            <div class="col-lg-5 col-md-12 col-sm-12 mb-3">
                                ...
                                <input type="text" class="form-control" id="username1" name="name" value="{{ old('name') }}">
                                @if ($errors->has('name'))
                        ...
                        <div class="mb-4">
                            <div class="col-lg-5 col-md-12 col-sm-12">
                                ...
                                <select class="form-control" id="role" name="group_id" value="{{ old('group_id') }}">
                                    @foreach ($groups as $g)
                                    <option value="{{ $g->group_id }}" {{ old('group_id') == $g->group_id ? 'selected' : '' }}>{{ $g->group_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="mb-4">
                            <div class="col-lg-5 col-md-12 col-sm-12">
                                ...
                                <select class="form-control" name="is_admin" value="{{ old('is_admin') }}">
                                    <option value="2" {{ old('is_admin') == "2" ? 'selected' : '' }}>一般社員</option>
                                    <option value="3" {{ old('is_admin') == "3" ? 'selected' : '' }}>グループ主任</option>
                                    <option value="1" {{ old('is_admin') == "1" ? 'selected' : '' }}>管理者</option>
                                    <option value="4" {{ old('is_admin') == "4" ? 'selected' : '' }}>外部利用者</option>
                                </select>
                            </div>
                        </div>
                        <div class="mb-4">
